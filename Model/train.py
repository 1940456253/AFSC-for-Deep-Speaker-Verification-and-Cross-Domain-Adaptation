...
# Make `from Dataset...`, `from Features...`, `from Model...` work when this
# file is run directly (e.g. PyCharm Run) instead of via `python -m Model.train`.
...
def make_optimizer(model, classifier, cfg, stage):
    ...
    groups = [{'params': list(model.encoder.parameters()) + list(classifier.parameters()), 'lr_scale': 1.0, 'weight_decay': cfg['weight_decay']}]
    if stage == 'joint':
        groups.append({'params': list(model.frontend.parameters()), 'lr_scale': cfg['gap_lr_multiplier'], 'weight_decay': 0.0})
    return torch.optim.SGD(groups, lr=cfg['learning_rate'], momentum=cfg['momentum'], nesterov=cfg['momentum'] > 0)
...
def rng_state(generator):
    ...
    return {'python': random.getstate(), 'numpy': [state[0], state[1].tolist(), state[2], state[3], state[4]], 'torch': torch.get_rng_state(), 'loader': generator.get_state(),
...
def main():
    default_cfg = PROJECT_ROOT / 'Model' / 'demo.yaml'
    default_csv = PROJECT_ROOT / 'demo_data' / 'train.csv'
    default_out = PROJECT_ROOT / 'runs' / 'demo'

    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument('--config', default=str(default_cfg), help=f'YAML recipe (default: {default_cfg})')
    p.add_argument('--train-csv', default=str(default_csv), help=f'Training manifest CSV (default: {default_csv})')
    p.add_argument('--output', default=str(default_out), help=f'Output directory (default: {default_out})')
    ...
    if not train_csv.is_file():
        raise FileNotFoundError(f'Training CSV not found: {train_csv}\n'
                                f'Generate it first, e.g.: python -m Dataset.demo')
    ...
    dataset = SpeakerDataset(str(train_csv), build_extractor(cfg['feature']), cfg['crop_seconds'], cfg['sample_rate'])
    if len(dataset.speakers) < 2:
        raise ValueError('Need at least two training speakers')
    loader = DataLoader(dataset, batch_size=cfg['batch_size'], shuffle=True, num_workers=cfg['num_workers'], drop_last=True, worker_init_fn=seed_worker, generator=generator,
    ...
    for epoch in range(epoch_start, end + 1):
        ...
        for step, (features, labels) in enumerate(loader):
            ...
            if step % cfg['log_every'] == 0:
                print(f'epoch={epoch}/{total} stage={stage} step={step}/{len(loader)} '
                      f'loss={loss.item():.4f} lr={base_lr:.6g} margin={margin:.4f}', flush=True)
        record = {'epoch': epoch, 'stage': stage, 'loss': losses / count, 'accuracy': correct / count, 'lr': base_lr, 'margin': margin, 'samples': count}
        with (out / 'train.jsonl').open('a') as f:
            f.write(json.dumps(record) + '\n')
        payload = {'format_version': 1, 'config': cfg, 'epoch': epoch, 'stage': stage, 'model': model.state_dict(), 'classifier': classifier.state_dict(), 'optimizer': optimizer.state_dict(),
                   'speakers': dataset.speakers, 'manifest_rows': dataset.rows, 'rng': rng_state(generator)}
