...
このプロジェクトは、論文の第一部分である AFSC 特徴、ECAPA 学習、埋め込み抽出、話者照合を扱い、MFCC、FBank の比較も含みます。Res2Net や X-Vector の公式コードも含みます。クロスドメイン適応は含まれず、コードは再学習可能で、論文が提供する CN-Celeb2 事前学習モデルの読み込みにも対応しています。
